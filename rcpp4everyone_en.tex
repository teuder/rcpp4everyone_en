...
Assigning a value to a ``reference'' object of a matrix is equivalent to
assigning the value to its original matrix. For example, assigning a
value to \texttt{col} will assign a value to the column 1 of \texttt{m}.
...
\texttt{Date} is a scalar type corresponding to an element of
\texttt{DateVector}.
...
\texttt{Date} has operators
\texttt{+,\ -,\ \textless{},\ \textgreater{},\ \textgreater{}=,\ \textless{}=,\ ==,\ !=}.
By using these operators, you can perform addition of days (\texttt{+}),
difference calculation of date (\texttt{-}), comparison of dates
...
When creating a Datetime object in the form of
\texttt{Datetime\ dt(str,\ format)}, \texttt{str} is interpreted as the
time in the local timezone. For example, if you run
\texttt{Datetime\ dt(2000-01-01\ 00:00:00);} in Japan Standard Time
(JST), the value of \texttt{1999-12-31\ 15:00:00\ UTC} is set
internally.
...
The
\texttt{+,\ -,\ \textless{},\ \textgreater{},\ \textgreater{}=,\ \textless{}=,\ ==,\ !=}
operators are defined in \texttt{Datetime}.

By using these operators, you can perform addition of seconds
(\texttt{+}), difference of datetime (\texttt{-}) in seconds, comparison
of datetime
(\texttt{\textless{},\ \textless{}=,\ \textgreater{},\ \textgreater{}=,\ ==,\ !=})
.
...
Note: The value output using these member functions is the time
interpreted at the time of Coordinated Universal Time. Therefore, it
looks different from the date and time of the user's time zone. (For
example, refer to the execution result of the code at the end of this
chapter)
...
\item
  \protect\hyperlink{functions-related-to-finding-values}{Functions
  related to finding values}
\item
  \protect\hyperlink{functions-related-to-duplicated-values}{Functions
  related to duplicated values}
\item
  \protect\hyperlink{functions-related-to-set-operation}{Functions
  related to set operation}
\item
  \protect\hyperlink{functions-related-to-maximum-and-minimum-values}{Functions
  related to maximum and minimum values}
\item
  \protect\hyperlink{functions-related-to-summaries}{Functions related
  to summaries}
\item
  \protect\hyperlink{functions-related-to-rounding-values}{Functions
  related to rounding values}
\item
  \protect\hyperlink{functions-related-to-math}{Functions related to
  math}
\item
  \protect\hyperlink{functions-related-to-logical-values}{Functions
  related to logical values}
\item
  \protect\hyperlink{functions-related-to-na-inf-nan}{Functions related
  to NA Inf NaN}
\item
...
\hypertarget{vector-related-functions}{\subsection{Vector related
functions}\label{vector-related-functions}}

\textbf{head(v, n)}

Returns a vector of \texttt{n} elements from the first element of the
vector \texttt{v}.

\textbf{tail(v, n)}

Returns a vector of last \texttt{n} elements from the last element of
vector \texttt{v}.

\textbf{rev(v)}

Returns a vector that elements of vector \texttt{v} are arranged in
reverse order.

\textbf{rep(x, n)}

Returns a vector that \texttt{x} is repeated \texttt{n} times.
\texttt{x} can be a scalar or vector.

\textbf{rep\_each(v, n)}

Returns a vector that each elements of vector v are repeated \texttt{n}
times.

\textbf{rep\_len(v, n)}

Returns a vector that the vector \texttt{v} is repeated until the length
of the vector becomes \texttt{n}.

\textbf{seq(start, end)}

Returns a vector of consecutive integers from \texttt{start} to
\texttt{end}.

\textbf{seq\_along(v)}

Returns a vector of consecutive integers from 1 to the number of
elements of vector \texttt{v}
...
Returns a vector that computed \texttt{v{[}i\ +\ 1{]}\ -\ v{[}i{]}} for
each element \texttt{i} excluding the last element of the vector
\texttt{v}.

\hypertarget{string-related-functions}{\subsection{String related
functions}\label{string-related-functions}}

\textbf{collapse(v)}

Returns a string concatenated with each element of the
\texttt{CharacterVector} v as \texttt{String} type.

\subsection{Finding values}\label{finding-values}

\textbf{match(v, table)}

Returns an integer vector containing the R style numerical index
(starting from 1) of the element of vector \texttt{table} that match
value to each elements of vector \texttt{x}. Namely、if you execute
\texttt{res\ =\ match(v,\ table);}, then it will be
\texttt{res{[}i{]}\ ==\ \ j+1} where \texttt{j} equals to minimum
\texttt{j} satisfying \texttt{v{[}i{]}\ ==\ table{[}j{]}}
...
Returns a vector containing 1 if the value of each element of vector v
exists in the previous element, containing 0 if not.

\textbf{unique(v)}

Returns a vector that eliminates the duplication of the element value
from the vector \texttt{v}.

\textbf{sort\_unique(v)}

Returns a vector that eliminates the duplication of the element value
from the vector \texttt{v} and sorts the values in ascending order.

\subsection{Set operation}\label{set-operation}

\textbf{setdiff(v1,v2)}

Returns a vector obtained by subtracting the value of the unique element
of the vector \texttt{v2} from the unique element of the vector
\texttt{v1}.

\textbf{setequal(v1,v2)}

Returns true if the unique element of vector \texttt{v1} is equal to the
unique element of vector \texttt{v2}.

\textbf{intersect(v1,v2)}

Returns a vector containing elements contained in both the unique
element of vector \texttt{v1} and the unique element of vector
\texttt{v2}.

\textbf{union\_(v1,v2)}

Return vector which eliminated value duplication after combining
elements of vector \texttt{v1} and vector \texttt{v2}.

\hypertarget{functions-related-to-maximum-and-minimum-values}{\subsection{Functions
related to maximum and minimum
values}\label{functions-related-to-maximum-and-minimum-values}}
...
Compares the corresponding elements of vectors \texttt{v1} and
\texttt{v2}, and return a vector containing the smaller elements.

\textbf{pmax(v1,v2)}

Compares the corresponding elements of vectors \texttt{v1} and
\texttt{v2}, and return a vector containing the larger elements.

\textbf{range(v)}

Returns a vector consisting of the minimum and maximum values of vector
\texttt{v}.

\textbf{clamp(min, v, max)}

Returns a vector that the elements of vector \texttt{v} smaller than
\texttt{min} is replaced with \texttt{min} and the elements larger than
\texttt{max} is replaced with \texttt{max}.

\hypertarget{functions-related-to-summaries}{\subsection{Functions
related to summaries}\label{functions-related-to-summaries}}
...
Returns a named integer vector that counts the number of elements for
each unique element of vector \texttt{v}.

\hypertarget{functions-related-to-rounding-values}{\subsection{Functions
related to rounding values}\label{functions-related-to-rounding-values}}

\textbf{floor(v)}

Returns a vector containing the largest integer not greater than each
element of vector \texttt{v}.

\textbf{ceil(v)}

Returns a vector containing the largest integer not smaller than each
element of vector v.
...
Returns a vector obtained by rounding each element of the vector
\texttt{v} with the number of significant figure \texttt{digits}.

\textbf{trunc(v)}

Returns a vector with rounded down decimal places.

\hypertarget{functions-related-to-math}{\subsection{Functions related to
math}\label{functions-related-to-math}}

\textbf{sign(v)}

Returns a vector with the signs of the corresponding elements of
\texttt{v} (the sign of a real number is 1, 0, or -1 if the number is
positive, zero, or negative, respectively).

\textbf{abs(v)}

Returns a vector containing the absolute value of each element of vector
\texttt{v}.

\textbf{pow(v, n)}

Returns a vector by raising each element of the vector \texttt{v} to the
\texttt{n}th power.
...
Returns a vector by raising Napier number (e) to the power of value of
each element of the vector v.
...
Returns a vector containing the natural logarithmic of each element of
vector \texttt{v}.

\textbf{log10(v)}

Returns a vector containing the common logarithmic of each element of
vector \texttt{v}.
...
Returns a vector containing hyperbolic sine of each element of vector
\texttt{v}.
...
Returns a vector containing hyperbolic cosine of each element of vector
\texttt{v}.

\textbf{tan(v)}

Returns a vector containing tangent of each element of vector
\texttt{v}.

\textbf{tanh(v)}

Returns a vector containing hyperbolic tangent of each element of vector
\texttt{v}.

\textbf{acos(v)}

Returns a vector containing arccosine of each element of vector
\texttt{v}.

\textbf{asin(v)}

Returns a vector containing arcsine of each element of vector
\texttt{v}.

\textbf{atan(v)}

Returns a vector containing arctangent of each element of vector
\texttt{v}.

\textbf{gamma(v)}

Returns a vector obtained by transforming each element of vector
\texttt{v} with the gamma function.
...
Returns a vector obtained by transforming each element of vector
\texttt{v} with the first derivative function of \texttt{lgamma()}.

\textbf{trigamma(v)}

Returns a vector obtained by transforming each element of vector
\texttt{v} with the second derivative function of \texttt{lgamma()}.

\textbf{tetragamma(v)}

Returns a vector obtained by transforming each element of vector
\texttt{v} with the third derivative function of \texttt{lgamma()}.

\textbf{pentagamma(v)}

Returns a vector obtained by transforming each element of vector
\texttt{v} with the fourth derivative function of \texttt{lgamma()}.

\textbf{psigamma(v, deriv)}

Returns a vector obtained by transforming each element of vector
\texttt{v} with the \texttt{deriv}-th derivative function of
\texttt{lgamma()}.

\textbf{factrial(v)}

Returns a vector containing the factorial of the each element of vector
\texttt{v}.
...
Returns a vector obtained by calculating binomial coefficients using the
corresponding elements of real vector \texttt{vn} and integer vector
\texttt{vk}.
...
Returns a vector obtained by calculating the value of the beta function
using the corresponding elements of vectors \texttt{va} and \texttt{vb}.

\textbf{lbeta(va, vb)}

Synonymous with \texttt{log(beta(va,\ vb))}

\hypertarget{functions-related-to-logical-values}{\subsection{Functions
related to logical values}\label{functions-related-to-logical-values}}

\textbf{all(v)}

Receives a \texttt{LogicalVector} \texttt{v} and returns
\texttt{SingleLogicalResult} type meaning\texttt{TRUE} when all of
elements are \texttt{TRUE}.

\textbf{any(v)}

Receives a \texttt{LogicalVector} \texttt{v} and returns
\texttt{SingleLogicalResult} type meaning\texttt{TRUE} when any of
elements are \texttt{TRUE}.

\textbf{is\_true(x)}

Receives a return value of \texttt{all()} or \texttt{any()} and returns
bool type \texttt{true} if it means \texttt{TRUE}.

\textbf{is\_false(x)}

Receives a return value of \texttt{all()} or \texttt{any()} and returns
bool type \texttt{true} if it means \texttt{FALSE}.

\textbf{is\_na(x)}

Receives a return value of \texttt{all()} or \texttt{any()} and returns
bool type \texttt{true} if it means \texttt{NA}.

\textbf{ifelse(v, x1, x2)}

Receives the \texttt{LogicalVector} \texttt{v}, and returns a vector
containing the corresponding element of x1 or x2 when the element of v
is \texttt{TRUE} or \texttt{FALSE}, respectively. Although \texttt{x1}
and \texttt{x2} can be vectors or scalars, the length of the vector
needs to be equal to \texttt{v}.

\hypertarget{functions-related-to-na-inf-nan}{\subsection{Functions
related to NA Inf NaN}\label{functions-related-to-na-inf-nan}}
...
Returns a \texttt{LogicalVector} containing \texttt{TRUE} if
corresponding elements of vector \texttt{v} is not \texttt{Inf} nor
\texttt{-Inf} nor \texttt{NA}.

\textbf{is\_infinite(v)}

Returns a \texttt{LogicalVector} containing \texttt{TRUE} if
corresponding elements of vector \texttt{v} is \texttt{Inf} or
\texttt{-Inf} or \texttt{NA}.

\textbf{is\_na(v)}

Returns a \texttt{LogicalVector} containing \texttt{TRUE} if
corresponding elements of vector \texttt{v} is \texttt{NA} or
\texttt{NaN}.

\textbf{is\_nan(v)}

Returns a \texttt{LogicalVector} containing \texttt{TRUE} if
corresponding elements of vector \texttt{v} is \texttt{NaN}.

\hypertarget{apply-functions}{\subsection{apply
functions}\label{apply-functions}}

\textbf{lapply(x, fun)}

Applies a C++ function \texttt{fun} to each element of the vector
\texttt{x} and returns the result as \texttt{List}.

\textbf{sapply(x, fun)}

Applies a C++ function \texttt{fun} to each element of the vector
\texttt{x} and returns the result as \texttt{Vector}.

\textbf{mapply(x1, x2, fun2)}

Applies a C++ function \texttt{fun} that receives two arguments to each
corresponding elements of the vector \texttt{x1} and \texttt{x2} and
returns the result as \texttt{Vector}.

\textbf{mapply(x1, x2, x3, fun3)}

Applies a C++ function \texttt{fun} that receives three arguments to
each corresponding elements of the vector \texttt{x1} and \texttt{x2},
\texttt{x3} and returns the result as \texttt{Vector}.

\hypertarget{cbind-function}{\subsection{cbind
function}\label{cbind-function}}

\textbf{cbind(x1, x2,\ldots{})}

Takes \texttt{Vector} or \texttt{Matrix} \texttt{x1} and \texttt{x2},
\texttt{...} and combine by columns. And returns the result as
\texttt{Matrix} or \texttt{DataFrame}. You can pass up to 50 arguments.

\subsection{sampling}\label{sampling}

\textbf{Vector sample(Vector x, int size, replace = false, probs =
R\_NilValue)}

As with the \texttt{sample} function in R, this function takes a sample
from a vector \texttt{x}.
...
\item
  \texttt{replace} : should sampling be with replacement. default
  \texttt{true}.
\item
  \texttt{probs} : a vector that specify probability weights to be drawn
  for each elements of vector \texttt{x}. default \texttt{R\_NilValue}
  (same weight for all elements).
\end{itemize}
...
\textbf{sample(n, size, replace = TRUE, probs = NULL, one\_based =
TRUE)}
...
Rcpp provides all major probability distribution functions in R. Same as
R, four functions starting with the character d/p/q/r are defined for
...
\section{Basic structure of probability distribution
function}\label{basic-structure-of-probability-distribution-function}

In Rcpp, probability distribution functions with the same name are
defined in two namespaces, \texttt{R::} and \texttt{Rcpp::}. These
differences are that the function defined in \texttt{Rcpp::} namespace
returns a vector, while the function in the \texttt{R::} namespace
returns a scalar. Basically, the probability distribution functions
defined in the \texttt{Rcpp::} namespace has the same functionalities as
those in R. So normally you can use a function in the \texttt{Rcpp::}
namespace, but if you want a scalar value, it is better to use that
function in \texttt{R::} namespace because it's faster.

The basic structures of the probability distribution functions defined
in the \texttt{Rcpp::} namespace are shown below. In fact, the
definition of the probability distribution function of the
\texttt{Rcpp\ ::} namespace is not written directly in the source code
of Rcpp (because it is written using macros). But you can assume that
the function is defined like as below.
...
The basic structures of the probability distribution functions defined
in the \texttt{R::} namespace are shown below. It basically has the same
functionality as those defined in the \texttt{Rcpp::} namespace except
that it accepts and returns \texttt{double} type. In addition, the
arguments of the function do not have default values, so user must give
...
The arguments of the probability distribution function are described
below.
...
\item
  \textbf{par} : Parameter（the number of distribution parameters varies
  depending on the distribution）
\item
  \textbf{lower} : \texttt{true} : Calculate the probability of the
  region where the random variable is less than or equal to x,
  \texttt{false} : Calculate the probability of the region larger than x
\item
  \textbf{log} : true : probabilities p are given as log(p)
\end{itemize}

\section{List of probability distribution
functions}\label{list-of-probability-distribution-functions}

List of probability distribution functions provided by Rcpp is shown
below. Here, the names of the distribution parameters are matched with
those of R, so refer to the R help for details.
...
\item
  \protect\hyperlink{noncentral-beta-distribution}{Noncentral beta
  distribution}
\item
  \protect\hyperlink{chi-squared-distribution}{Chi-squared distribution}
\item
  \protect\hyperlink{noncentral-chi-squared-distribution}{Noncentral
  chi-squared distribution}
\item
  \protect\hyperlink{t-distribution}{t-distribution}
\item
  \protect\hyperlink{noncentral-t-distribution}{Noncentral
  t-distribution}
\item
  \protect\hyperlink{f-distribution}{F-distribution}
\item
  \protect\hyperlink{noncentral-f-distribution}{Noncentral
  F-distribution}
\item
...
\item
  \protect\hyperlink{negative-binomial-distribution-with-success-probability-as-parameter}{Negative
  binomial distribution (with success probability as parameter)})
\item
  \protect\hyperlink{negative-binomial-distribution-with-mean-as-parameter}{Negative
  binomial distribution (with mean as parameter)}
\item
...
\item
  \protect\hyperlink{Hypergeometricux5cux2520distribution}{Hypergeometric
  distribution}
\item
  \protect\hyperlink{distribution-of-wilcoxon-rank-sum-test-statistic}{Distribution
  of Wilcoxon rank-sum test statistic}
\item
  \protect\hyperlink{distribution-of-wilcoxon-signed-rank-test-statistic}{Distribution
  of Wilcoxon signed-rank test statistic}
\end{itemize}

\section{Continuous probability
distribution}\label{continuous-probability-distribution-1}

\hypertarget{uniform-distribution}{\subsection{Uniform
distribution}\label{uniform-distribution}}

These functions provide information about the uniform distribution on
the interval from \texttt{min} to \texttt{max}.
...
\hypertarget{normal-distribution}{\subsection{Normal
distribution}\label{normal-distribution}}

These functions provide information about the normal distribution with
mean equal to \texttt{mean} and standard deviation equal to \texttt{sd}.
...
\hypertarget{log-normal-distribution}{\subsection{Log-normal
distribution}\label{log-normal-distribution}}

These functions provide information about the log-normal distribution
whose logarithm has mean equal to \texttt{meanlog} and standard
deviation equal to \texttt{sdlog}.
...
\hypertarget{gamma-distribution}{\subsection{Gamma
distribution}\label{gamma-distribution}}

These functions provide information about the Gamma distribution with
...
\hypertarget{beta-distribution}{\subsection{Beta
distribution}\label{beta-distribution}}

These functions provide information about the Beta distribution with
parameters \texttt{shape1} and \texttt{shape2}. These functions are
equivalent to setting 0 for the noncentrality parameter \texttt{ncp} in
the Beta distribution function in R.
...
\hypertarget{noncentral-beta-distribution}{\subsection{Noncentral beta
distribution}\label{noncentral-beta-distribution}}

These functions provide information about the Noncentral beta
distribution with parameters \texttt{shape1} and \texttt{shape2},
noncentrality parameter \texttt{ncp}. These functions are equivalent to
setting non 0 value for the noncentrality parameter \texttt{ncp} in the
Beta distribution function in R.
...
\hypertarget{chi-squared-distribution}{\subsection{Chi-squared
distribution}\label{chi-squared-distribution}}

These functions provide information about the Chi-squared distribution
with df degrees of freedom \texttt{df}. These functions are equivalent
to setting 0 for the noncentrality parameter \texttt{ncp} in the Beta
...
\hypertarget{noncentral-chi-squared-distribution}{\subsection{Noncentral
chi-squared distribution}\label{noncentral-chi-squared-distribution}}

These functions provide information about the Noncentral chi-squared
distribution with df degrees of freedom \texttt{df} and noncentrality
parameter \texttt{ncp}. These functions are equivalent to setting non 0
value for the noncentrality parameter \texttt{ncp} in the Chi-squared
distribution function in R.
...
\hypertarget{t-distribution}{\subsection{t-distribution}\label{t-distribution}}

These functions provide information about the t-distribution with df
degrees of freedom \texttt{df}. These functions are equivalent to
setting 0 for the noncentrality parameter \texttt{ncp} in the Beta
distribution function in R.
...
\hypertarget{noncentral-t-distribution}{\subsection{Noncentral
t-distribution}\label{noncentral-t-distribution}}

These functions provide information about the Noncentral t-distribution
with df degrees of freedom \texttt{df} and noncentrality parameter
\texttt{ncp}. These functions are equivalent to setting non 0 value for
the noncentrality parameter \texttt{ncp} in the t-distribution function
in R.
...
\hypertarget{f-distribution}{\subsection{F-distribution}\label{f-distribution}}

These functions provide information about the F-distribution with df
degrees of freedom \texttt{df1} and \texttt{df2}. These functions are
equivalent to setting 0 for the noncentrality parameter \texttt{ncp} in
the F-distribution function in R.
...
\hypertarget{noncentral-f-distribution}{\subsection{Noncentral
F-distribution}\label{noncentral-f-distribution}}

These functions provide information about the F-distribution with df
degrees of freedom \texttt{df1}, \texttt{df2} and noncentrality
parameter \texttt{ncp}. These functions are equivalent to setting non 0
value for the noncentrality parameter \texttt{ncp} in the Noncentral
F-distribution function in R.
...
\hypertarget{cauchy-distribution}{\subsection{Cauchy
distribution}\label{cauchy-distribution}}

These functions provide information about the Cauchy distribution with
location parameter \texttt{location} and scale parameter \texttt{scale}.
...
\hypertarget{weibull-distribution}{\subsection{Weibull
distribution}\label{weibull-distribution}}

These functions provide information about the Weibull distribution with
parameters \texttt{shape} and \texttt{scale}.
...
distribution}\label{discrete-probability-distribution-1}

\hypertarget{binomial-distribution}{\subsection{Binomial
distribution}\label{binomial-distribution}}

These functions provide information about the Binomial distribution with
number of trials \texttt{size} and success probability \texttt{prob}.
...
\hypertarget{negative-binomial-distribution-with-success-probability-as-parameter}{\subsection{Negative
binomial distribution (with success probability as
parameter)}\label{negative-binomial-distribution-with-success-probability-as-parameter}}

These functions provide information about the Negative binomial
distribution with number of success \texttt{size} and success
probability \texttt{prob}.
...
\hypertarget{negative-binomial-distribution-with-mean-as-parameter}{\subsection{Negative
binomial distribution (with mean as
parameter)}\label{negative-binomial-distribution-with-mean-as-parameter}}

These functions provide information about the Negative binomial
distribution with number of success \texttt{size} and mean \texttt{mu}.
...
\hypertarget{poisson-distribution}{\subsection{Poisson
distribution}\label{poisson-distribution}}

These functions provide information about the Poisson distribution with
mean and variance are equal to \texttt{lambda}.
...
\hypertarget{geometric-distribution}{\subsection{Geometric
distribution}\label{geometric-distribution}}

These functions provide information about the Geometric distribution
with success probability \texttt{prob}.
...
distribution with number of success in the population \texttt{m} ,
number of failure in the population \texttt{n}, number of sample from
the population \texttt{k}.
...
\hypertarget{distribution-of-wilcoxon-rank-sum-test-statistic}{\subsection{Distribution
of Wilcoxon rank-sum test
statistic}\label{distribution-of-wilcoxon-rank-sum-test-statistic}}

These functions provide information about the distribution of test
statistic when Wilcoxon rank-sum test (Mann--Whitney U test) is
performed on two specimens with number of samples \texttt{m} and
\texttt{n} respectively.
...
\hypertarget{distribution-of-wilcoxon-signed-rank-test-statistic}{\subsection{Distribution
of Wilcoxon signed-rank test
statistic}\label{distribution-of-wilcoxon-signed-rank-test-statistic}}

These functions provide information about the distribution of test
statistic when Wilcoxon signed-rank test is performed with number of
samples \texttt{n}.
...
In order to use R functions in Rcpp, you can use \texttt{Function} and
\texttt{Environment}.

\section{Function}\label{function}

Using the \texttt{Function} class, you can call R functions from Rcpp.
The argument given to the R function is determined based on position and
name.

Use \texttt{Named()} or \texttt{\_{[}{]}} to pass a value to an argument
by specifying argument name. \texttt{Name()} can be used in two ways:
\texttt{Named("argument\_name",\ value)} or
\texttt{Named("argument\_name")\ =\ value}.

The code example below shows an example of calling the R function
\texttt{rnorm\ (n,\ mean,\ sd)}from the function defined in Rcpp.
However, when calling a package function using \texttt{Function} class,
you have to add the package environment to the search path using the
\texttt{library()} function in R in advance.
...
In the above example, the return type of R function called from Rcpp is
assumed \texttt{NumericVector}. However, as in the example below, the
return type of an R function called from Rcpp function is sometimes
undefined. In such a case it would be better to assign the return value
of the function into \texttt{RObject} or \texttt{List} element.

The code below shows an example of defining simplified R function
\texttt{lapply()} with Rcpp.
...
By using \texttt{Environment} class, you can retrieve objects (variables
and functions) from packages and other environments.

The code below shows an example of calling \texttt{Matrix()} function in
the \texttt{Matrix} package. When calling a package function in this
way, it is not necessary to attach the package using \texttt{library()}
function.
...
To express the value of \texttt{Inf} \texttt{-Inf} \texttt{NaN} in Rcpp,
use the symbol \texttt{R\_PosInf} \texttt{R\_NegInf} \texttt{R\_NaN}.
...
On the other hand, for \texttt{NA}, different symbol of \texttt{NA} are
defined for each \texttt{Vector} type.
...
The following code example shows how to use these symbols to create
\texttt{Vector} object.
...
\subsection{Evaluating all the elements of a vector at
once}\label{evaluating-all-the-elements-of-a-vector-at-once}

To evaluate all the \texttt{NA} \texttt{NaN} \texttt{Inf} \texttt{-Inf}
elements in a vector at once, use the function \texttt{is\_na()}
\texttt{is\_nan()} \texttt{is\_infinite()}.

In the code example below, we create a vector containing \texttt{NA}
\texttt{NaN} \texttt{Inf} \texttt{-Inf} and evaluate it. From this
example we can see that the \texttt{is\_na()} evaluates both \texttt{NA}
and\texttt{NaN} as \texttt{TRUE} (same as R's \texttt{is.na()}).
...
You can remove \texttt{NA} \texttt{NaN} \texttt{Inf} from a vector by
using these functions. You can also use \texttt{na\_omit()} to remove
\texttt{NA}.

The code example below shows how to remove \texttt{NA} from a vector
using the \texttt{is\_na()} and \texttt{na\_omit()}.
...
\subsection{Evaluating single element of a
vector}\label{evaluating-single-element-of-a-vector}

If you want to evaluate \texttt{NA} \texttt{NaN} \texttt{Inf}
\texttt{-Inf} on single element of a vector, use the static member
function \texttt{Vector::is\_na()},
\texttt{traits::is\_nan\textless{}RTYPE\textgreater{}()},
\texttt{traits::\ is\_infinite\textless{}RTYPE\textgreater{}()}. In
\texttt{RTYPE}, specify \texttt{SEXPTYPE} of the vector to be evaluated.
...
You use \texttt{R\_NilValue} to handle\texttt{NULL} in Rcpp. The code
example below shows how to check \texttt{NULL} in the elements of a
\texttt{List} object and how to assign \texttt{NULL} to clear the value
of an attribute.
...
\section{Points to note when handling NA with
Rcpp}\label{points-to-note-when-handling-na-with-rcpp}

Internally \texttt{NA\_INTEGER} and \texttt{NA\_LOGICAL} are equivalent
to the minimum value of \texttt{int} (-2147483648). Although, functions
and operators defined in Rcpp handle the minimum value of \texttt{int}
appropriately as\texttt{NA} (that is, make the result of operation on
\texttt{NA} element as \texttt{NA}). Standard C++ functions and
operators treat the minimum value of \texttt{int} as integer value. So
if you add 1 to \texttt{NA\_INTEGER}, the element is no longer minimum
value of \texttt{int}, so it is not　treated as \texttt{NA}.

In addition, if you assign \texttt{NA\_LOGICAL} to \texttt{bool} type,
it will always be evaluated as \texttt{true}. This is because
\texttt{bool} evaluates all numbers other than 0 as \texttt{true}.

On the other hand, since \texttt{nan} and　\texttt{inf} are defined in
\texttt{double}, the result of the operation on　\texttt{nan}
\texttt{inf} in standard C ++ will be the same result as R.

The table below summarizes how values are evaluated when assigning the
value of \texttt{NA} \texttt{Inf} \texttt{-Inf} \texttt{NaN} to
\texttt{Vector} or scalar type in C++.
...
\midrule
\endhead
\texttt{NumericVector} & \texttt{NA\_REAL} & \texttt{R\_NaN} &
\texttt{R\_PosInf} & \texttt{R\_NegInf}\tabularnewline
\texttt{IntegerVector} & \texttt{NA\_INTEGER} & \texttt{NA\_INTEGER} &
\texttt{NA\_INTEGER} & \texttt{NA\_INTEGER}\tabularnewline
\texttt{LogicalVector} & \texttt{NA\_LOGICAL} & \texttt{NA\_LOGICAL} &
\texttt{NA\_LOGICAL} & \texttt{NA\_LOGICAL}\tabularnewline
\texttt{CharacterVector} & \texttt{NA\_STRING} & ``NaN'' & ``Inf'' &
``-Inf''\tabularnewline
\texttt{String} & \texttt{NA\_STRING} & ``NaN'' & ``Inf'' &
``-Inf''\tabularnewline
\texttt{double} & \texttt{nan} & \texttt{nan} & \texttt{inf} &
\texttt{-inf}\tabularnewline
\texttt{int} & -2147483648 & -2147483648 & -2147483648 &
-2147483648\tabularnewline
\texttt{bool} & \texttt{true} & \texttt{true} & \texttt{true} &
\texttt{true}\tabularnewline
\bottomrule
\end{longtable}

The code example below shows the difference in results when computing
using the Rcpp operator and the standard C++ operator against
\texttt{NA\_INTEGER}. From this result, the operator of Rcpp evaluates
the result of the operation on \texttt{NA} as \texttt{NA}, but you can
see that the standard C++ operator treats \texttt{NA\_INTEGER} as just a
integer value.
...
The factor vector (\texttt{factor}) is actually an integer vector with
the attributes \texttt{levels} and \texttt{class} is defined.

In the code below, an example of converting integer vector to
\texttt{factor} by setting values to attributes.
...
The execution result below, we can see that the integer vector returned
to R is treated as \texttt{factor}.
...
In a situation in which the normal progress of the program is hindered,
you can print an error message and stop the program. If you want to warn
the user without stopping the program, use \texttt{warning()} function.
Both functions \texttt{stop()} and \texttt{warning()} can display
messages by specifying the format as same as the \texttt{Rprintf()}
function.
...
In the code example below, an error is printed and execution is stopped
if the value given to the function is negative.
...
\texttt{CheckUserInterrupt()} function checks if the `ctrl + c' button
was pressed, and if it was pressed it will stop the execution of the
function.
...
The probability distribution function is also described on the page
\href{21_dpqr_functions.md}{Probability distributions}.
